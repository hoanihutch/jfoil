...
"""
function get_H(U::AbstractVector)
    H = U[2] / U[1]
    H_U = [-H / U[1], 1.0 / U[1], 0.0, 0.0]
    return H, H_U
end
...
"""
function get_Hw(U::AbstractVector, wgap::Real)
    Hw = wgap / U[1]
    Hw_U = [-Hw / U[1], 0.0, 0.0, 0.0]
    return Hw, Hw_U
end
...
"""
function get_Mach2(U::AbstractVector, param)
    if param.Minf > 0
...
"""
function get_Hk(U::AbstractVector, param)
    H, H_U = get_H(U)
...
"""
function get_Hs(U::AbstractVector, param)
    ...
    if param.turb
        ...
        Reb = Ret; Reb_U = copy(Ret_U)
        if Ret < 200
            Reb = 200.0; Reb_U = Reb_U .* 0.0
        end
...
"""
function get_Hss(U::AbstractVector, param)
    M2, M2_U = get_Mach2(U, param)
    Hk, Hk_U = get_Hk(U, param)
...
"""
function get_Ret(U::AbstractVector, param)
    if param.Minf > 0
        M2, M2_U = get_Mach2(U, param)
        uk, uk_u = get_uk(U[4], param)
...
"""
function get_rho(U::AbstractVector, param)
    if param.Minf > 0
        M2, M2_U = get_Mach2(U, param)
        gmi = param.gam - 1.0
        den = 1.0 + 0.5 * gmi * M2; den_M2 = 0.5 * gmi
...
"""
function get_de(U::AbstractVector, param)
    Hk, Hk_U = get_Hk(U, param)
    aa = 3.15 + 1.72 / (Hk - 1.0)
